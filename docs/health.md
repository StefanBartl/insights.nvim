...
Fifteen sections. Most degrade gracefully — a missing optional tool is
...
| Todo comments | Skipped (`info`) if `todos.enable = false`; otherwise the keyword and word counts, a `warn` per keyword whose colour category resolves to no colour, `rg` for the scan (`error` if missing) with the UI `todos.search.ui` resolves to, and whether the in-buffer highlight is on |
...
| `Insights: routes` | Generated by `lib.nvim.bindings.usercmd.composer.checkhealth` — one line per registered `:Insights` route/subcommand |
